...
static int rcar_vcon_crtc_get(struct rcar_vcon_crtc *rcrtc)
{
	...
	ret = clk_prepare_enable(rcrtc->clk);
	if (ret < 0)
		return ret;
...
{
	clk_disable_unprepare(rcrtc->clk);
...
int rcar_vcon_crtc_create(struct rcar_vcon_device *rvcon, unsigned int index)
{
	...
	rcrtc->clk = devm_clk_get(rvcon->dev, clk_name);
	if (IS_ERR(rcrtc->clk)) {
		dev_err(rvcon->dev, "no clock for CRTC %u\n", index);
		return PTR_ERR(rcrtc->clk);
	}

	rcrtc->dclk_src = clk_get_rate(rcrtc->clk);
