...
static int renesas_pcie_establish_link(struct dw_pcie *pci)
{
	struct renesas_pcie *pcie = to_renesas_pcie(pci);
	u32 val;
...
static const struct dw_pcie_ops dw_pcie_ops = {
	.start_link = renesas_pcie_establish_link,
...
static int renesas_pcie_host_init(struct pcie_port *pp)
{
	...
	/* Software reset released - CPG setting */
	...
	/* Start link training, stop reprogramming of PHY, and check link up */
	ret = renesas_pcie_establish_link(pci);
	if (ret) {
				      ...
				      struct platform_device *pdev)
{
	...
	/* Renesas specific application registers */
	...
	if (IS_ERR(pcie->bus_clk)) {
		dev_err(dev, "cannot get Reference clock\n");
		return PTR_ERR(pcie->bus_clk);
	}

	pcie->axi_clk = devm_clk_get(dev, "pcie_axi");
	if (IS_ERR(pcie->axi_clk)) {
		dev_err(dev, "cannot get AXI bus clock\n");
		return PTR_ERR(pcie->axi_clk);
	}

	pcie->apb_clk = devm_clk_get(dev, "pcie_apb");
	if (IS_ERR(pcie->apb_clk)) {
		dev_err(dev, "cannot get APB bus clock\n");
		return PTR_ERR(pcie->apb_clk);
	}

	pcie->phy_clk = devm_clk_get(dev, "pcie_phy");
	if (IS_ERR(pcie->phy_clk)) {
		dev_err(dev, "cannot get PHY ctrl regs clock\n");
		return PTR_ERR(pcie->phy_clk);
	}

	pcie->rst = devm_reset_control_get(dev, NULL);
	if (IS_ERR(pcie->rst)) {
		dev_err(dev, "failed to get CPG reset\n");
...
static int renesas_pcie_probe(struct platform_device *pdev)
{
	...
	enum dw_pcie_device_mode mode;
	/* Enable Reference Clock - PFC setting */
	void __iomem *gpsr;
	void __iomem *pmmr;
	u32 val;

	dev_info(dev, "GPSR set\n");
	gpsr = ioremap_nocache(0xe6060040, 0x04);
	val = readl_relaxed(gpsr);
	dev_info(dev, "GPSR: %x: val before WRITE\n", val);

	val = val | 0x01e00000;

	dev_info(dev, "PMMR set\n");
	pmmr = ioremap_nocache(0xe6060000, 0x04);
	writel_relaxed(~val, pmmr);	/* write ~val to pmmr */
	dev_info(dev, "PMMR set DONE\n");

	dev_info(dev, "GPSR: %x: val to WRITE\n", val);
	writel_relaxed(val, gpsr);
	dev_info(dev, "GPSR set DONE\n");
	val = readl_relaxed(gpsr);
	dev_info(dev, "GPSR: %x: val after WRITE\n", val);
	iounmap(gpsr);
	iounmap(pmmr);
	/* Done PFC setting */
	...
	err = clk_prepare_enable(pcie->axi_clk);
	if (err) {
		dev_err(dev, "failed to enable AXI bus clock: %d\n", err);
		goto err_pm_put;
	}

	err = clk_prepare_enable(pcie->apb_clk);
	if (err) {
		dev_err(dev, "failed to enable APB bus clock: %d\n", err);
		goto err_pm_put;
	}

	err = clk_prepare_enable(pcie->phy_clk);
	if (err) {
		dev_err(dev, "failed to enable PHY ctrl regs clock: %d\n", err);
		goto err_pm_put;
	}
...
err_clk_disable:
	...
	clk_disable_unprepare(pcie->axi_clk);
	clk_disable_unprepare(pcie->apb_clk);
	clk_disable_unprepare(pcie->phy_clk);
