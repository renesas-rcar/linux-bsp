 ...
 */
struct r8a779g0_sysc_info {
	const struct r8a779g0_sysc_area *areas;
	unsigned int num_areas;
};

static struct r8a779g0_sysc_area r8a779g0_areas[] __initdata = {
	...
	{ "3dg-a",	R8A779G0_PD_A33DGA, R8A779G0_PD_ALWAYS_ON },
	{ "3dg-b",	R8A779G0_PD_A23DGB, R8A779G0_PD_A33DGA },
...
static void __iomem *r8a779g0_sysc_base;
static DEFINE_SPINLOCK(r8a779g0_sysc_lock); /* SMP CPUs + I/O devices */

static int r8a779g0_sysc_pwr_on_off(u8 pdr, bool on)
{
	...
	ret = readl_poll_timeout_atomic(r8a779g0_sysc_base + SYSCSR, val,
	...
	iowrite32(PWRON_PWROFF, r8a779g0_sysc_base + reg_offs);
...
static int clear_irq_flags(unsigned int reg_idx, unsigned int isr_mask)
{
	...
	iowrite32(isr_mask, r8a779g0_sysc_base + SYSCISCR(reg_idx));

	ret = readl_poll_timeout_atomic(r8a779g0_sysc_base + SYSCISCR(reg_idx),
					val, !(val & isr_mask),
...
static int r8a779g0_sysc_power(u8 pdr, bool on)
{
	...
	spin_lock_irqsave(&r8a779g0_sysc_lock, flags);
	 ...
	 */
	iowrite32(ioread32(r8a779g0_sysc_base + SYSCIER(reg_idx)) | isr_mask,
		  r8a779g0_sysc_base + SYSCIER(reg_idx));
	iowrite32(ioread32(r8a779g0_sysc_base + SYSCIMR(reg_idx)) | isr_mask,
		  r8a779g0_sysc_base + SYSCIMR(reg_idx));
	...
	for (k = 0; k < PDRESR_RETRIES; k++) {
		ret = r8a779g0_sysc_pwr_on_off(pdr, on);
		if (ret)
			goto out;

		status = ioread32(r8a779g0_sysc_base + PDRESR(pdr));
 ...
 out:
	spin_unlock_irqrestore(&r8a779g0_sysc_lock, flags);

	pr_debug("sysc power %s domain %d: %08x -> %d\n", on ? "on" : "off",
		 pdr, ioread32(r8a779g0_sysc_base + SYSCISCR(reg_idx)), ret);
	return ret;
}

static bool r8a779g0_sysc_power_is_off(u8 pdr)
{
	unsigned int st;

	st = ioread32(r8a779g0_sysc_base + PDRSR(pdr));
...
static inline struct r8a779g0_sysc_pd *to_r8a779g0_pd(struct generic_pm_domain *d)
{
	return container_of(d, struct r8a779g0_sysc_pd, genpd);
}

static int r8a779g0_sysc_pd_power_off(struct generic_pm_domain *genpd)
{
	struct r8a779g0_sysc_pd *pd = to_r8a779g0_pd(genpd);

	pr_debug("%s: %s\n", __func__, genpd->name);
	return r8a779g0_sysc_power(pd->pdr, false);
}

static int r8a779g0_sysc_pd_power_on(struct generic_pm_domain *genpd)
{
	struct r8a779g0_sysc_pd *pd = to_r8a779g0_pd(genpd);
...
static int __init r8a779g0_sysc_pd_setup(struct r8a779g0_sysc_pd *pd)
{
	...
	genpd->power_off = r8a779g0_sysc_pd_power_off;
	genpd->power_on = r8a779g0_sysc_pd_power_on;
	...
	if (!r8a779g0_sysc_power_is_off(pd->pdr)) {
		pr_debug("%s: %s is already powered\n", __func__, genpd->name);
		goto finalize;
	}

	r8a779g0_sysc_power(pd->pdr, true);
...
static const struct of_device_id r8a779g0_sysc_matches[] __initconst = {
	{ .compatible = "renesas,r8a779g0-sysc", .data = &r8a779g0_sysc_info },
	{ /* sentinel */ }
};

struct r8a779g0_pm_domains {
	struct genpd_onecell_data onecell_data;
	struct generic_pm_domain *domains[R8A779G0_PD_ALWAYS_ON + 1];
};

static struct genpd_onecell_data *r8a779g0_sysc_onecell_data;

static int __init r8a779g0_sysc_pd_init(void)
{
	const struct r8a779g0_sysc_info *info;
	const struct of_device_id *match;
	struct r8a779g0_pm_domains *domains;
...
early_initcall(r8a779g0_sysc_pd_init);
