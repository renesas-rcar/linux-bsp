					      ...
					      enum dma_slave_buswidth bus_width)
{
	...
	if (IS_ERR(chan)) {
		dev_err(dev, "request_channel failed for %s (%ld)\n",
					 ...
					 enum dma_slave_buswidth bus_width)
{
	...
	if (dir == DMA_DEV_TO_MEM || dir == DMA_MEM_TO_DEV) {
		...
		dev_err(dev, "%s: FAILED for dir=%d\n, chan %s", __func__, dir, chan_name);
			     ...
			     void *data, unsigned int len)
{
	...
	if (dma_submit_error(cookie)) {
		dev_err(dev, "%s: submit TX dma failed, using PIO\n", __func__);
			     ...
			     void *data, unsigned int len)
{
	...
	if (dma_submit_error(cookie)) {
		dev_err(dev, "%s: submit RX dma failed, using PIO\n", __func__);
		rcar_wcrc_cleanup_dma(priv);
		return false;
	}
...
static int wcrc_setting_e2e_crc(struct wcrc_info *info, struct wcrc_device *priv)
{
	...
	if (IS_ERR(priv->dma_tx) | IS_ERR(priv->dma_rx)) {
		ret = -EINVAL;
		goto end_mode;
	}
...
static int wcrc_setting_data_thr(struct wcrc_info *info, struct wcrc_device *priv)
{
	...
	if (IS_ERR(priv->dma_tx) | IS_ERR(priv->dma_rx_in)) {
		ret = -EINVAL;
		goto end_mode;
	}
...
static int wcrc_setting_e2e_data_thr(struct wcrc_info *info, struct wcrc_device *priv)
{
	...
	if (IS_ERR(priv->dma_tx) | IS_ERR(priv->dma_rx_in) | IS_ERR(priv->dma_rx)) {
		ret = -EINVAL;
		goto end_mode;
	}
