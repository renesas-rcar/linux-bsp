...
static int8_t *remaped_log_buffer;
...
static int32_t rcar_optee_add_suspend_callback(void);
static void rcar_optee_del_suspend_callback(void);
static int32_t rcar_optee_init_debug_log(void);
...
static int tz_rcar_suspend(void)
{
	...
	smc_req_list = list_empty(&optee->call_queue.waiters);
...
int32_t optee_rcar_probe(struct optee *optee)
{
	int32_t ret;
