...
#define FUNC_CLR_ALL		(MH_TX_FQ0_IRQ | MH_TX_FQ1_IRQ | MH_TX_FQ2_IRQ |\
				 MH_TX_FQ3_IRQ | MH_TX_FQ4_IRQ | MH_TX_FQ5_IRQ |\
				 MH_TX_FQ6_IRQ | MH_TX_FQ7_IRQ | MH_RX_FQ0_IRQ |\
				 MH_RX_FQ1_IRQ | MH_RX_FQ2_IRQ | MH_RX_FQ3_IRQ |\
				 MH_RX_FQ4_IRQ | MH_RX_FQ5_IRQ | MH_RX_FQ6_IRQ |\
				 MH_RX_FQ7_IRQ | MH_TX_PQ_IRQ  | MH_STOP_IRQ   |\
				 MH_RX_FILTER_IRQ | MH_TX_FILTER_IRQ | MH_TX_ABORT_IRQ |\
				 MH_RX_ABORT_IRQ | MH_STATS_IRQ | PRT_E_ACTIVE |\
				 PRT_BUS_ON | PRT_TX_EVT | PRT_RX_EVT)
...
#define ERR_CLR_ALL		(MH_RX_FILTER_ERR | MH_MEM_SFTY_ERR | MH_REG_CRC_ERR |\
				 MH_DESC_ERR | MH_AP_PARITY_ERR | MH_DP_PARITY_ERR |\
				 MH_DP_SEQ_ERR | MH_DP_DO_ERR | MH_DP_TO_ERR | MH_DMA_TO_ERR |\
				 MH_DMA_CH_ERR | MH_RD_RESP_ERR | MH_WR_RESP_ERR |\
				 MH_MEM_TO_ERR | PRT_ABORTED | PRT_USOS | PRT_TX_DU |\
				 PRT_RX_DO | PRT_IFF_RQ | PRT_BUS_ERR | PRT_E_PASSIVE |\
				 PRT_BUS_OF | TOP_MUX_TO_ERR)
...
static void rcar_canxl_enable_interrupts(struct rcar_canxl_global *gpriv)
{
	...
	val = rcar_canxl_read(gpriv->base, FUNC_RAW);
	if (val != 0)
		pr_err("FUNC_RAW register not cleared\n");

	val = rcar_canxl_read(gpriv->base, ERR_RAW);
	if (val != 0)
		pr_err("ERR_RAW register not cleared\n");
						 ...
						 *gpriv)
{
	...
	rcar_canxl_write(gpriv->base, FUNC_CLR, 0xFFFFFFFF);
	rcar_canxl_write(gpriv->base, ERR_CLR, 0xFFFFFFFF);
	u32 val = rcar_canxl_read(gpriv->base, FUNC_RAW);
				 ...
				 u16 txerr, u16 rxerr)
{
	...
	val = rcar_canxl_read(priv->base, ERR_RAW);
	if (val != 0)
		pr_err("ERR_RAW register not cleared\n");
...
static void rcar_canxl_set_bittiming(struct net_device *dev)
{
	...
	if (priv->gpriv->xlmode) {
		cfg = (XBTP_XTSEG1(tseg1) |
		       XBTP_XSJW(sjw) |
		       XBTP_XTSEG2(tseg2));
		rcar_canxl_write(priv->base, XBTP, cfg);
	} else {
		cfg = (DBTP_DTSEG1(tseg1) |
		       DBTP_DSJW(sjw) |
		       DBTP_DTSEG2(tseg2));
		rcar_canxl_write(priv->base, DBTP, cfg);
	}
...
static int rcar_canxl_start(struct net_device *ndev)
{
	...
	if (err) {
		dev_err(&pdev->dev, "Start PRT failed\n");
		goto fail_mode_change;
	}
...
{
	struct rcar_canxl_channel *priv = netdev_priv(ndev);
	struct canxl_frame *cxl = NULL;
	struct canfd_frame *cfd = NULL;
	u16 id, sdt, dlc, rc, crc, xtd, xlf = 1, sec = 0, brs = 0, esi = 0, fdf = 0;
	u32 af, pay_load_size, target_desc_index;
	u32 ele0, ele1, ele4_t0, ele5_t1, ele7_txap, cfg;
	uintptr_t ele7_txap_virt;
	int ret;
	unsigned long flags;
	struct rcar_canxl_global *gpriv = priv->gpriv;

	if (gpriv->xlmode)
		cxl = (struct canxl_frame *)skb->data;
	else
	...
	if (gpriv->xlmode) {
		...
		uintptr_t ele6_td0t2 = TX_FQ_STADD(gpriv->sys_base, 0)
...
static void rcar_canxl_remove(struct platform_device *pdev)
{
