...
static int rtsn_parse_rswitch(struct rtsn_private *priv)
{
	...
	if (priv->rswitch_port > RTSN_RSWITCH_IF_MAX)
		return dev_err_probe(dev, -EINVAL,
				"invalid rswitch interface %u for tsnes%u\n",
				priv->rswitch_port, priv->tsnes_id);

	ret = rtsn_parse_rswitch_forward_ports(priv);
	if (ret)
		return ret;
...
static int rtsn_attach_rswitch(struct rtsn_private *priv)
{
	...
	ret = rswitch_attach_tsnes(priv->rswitch_dev, priv->tsnes_id,
					priv->rswitch_port,
...
static void rtsn_detach_rswitch(struct rtsn_private *priv)
{
	...
	if (priv->rswitch_dev)
		rswitch_detach_tsnes(priv->rswitch_dev, priv->tsnes_id, priv->rswitch_port);
				   ...
				   struct net_device *ndev)
{
	...
	/*
	 * Try to read address from device. On the RFS2X virtual platform the
	 * RMAC is in DISABLE mode here, so the read is invalid and only emits
	 * a warning; skip it and fall back to a random address.
	 */
	if (!priv->vpf_mode) {
...
static int rtsn_probe(struct platform_device *pdev)
{
	...
	if (of_find_property(pdev->dev.of_node, "renesas,vpf-mode", NULL))
		priv->vpf_mode = true;
	else
		priv->vpf_mode = false;
