	...
	if (chan->protocol_id == PHY_MODE_PCIE) {
		...
		case 4:
			if(channel_id == 0) {
				...
				if (data) {
					dev_err(priv->dev,"Timeout waiting for MPPHY_PXSRAMCNT(0) configuration\n");
				}
				data = mp_phy_reg_wait(priv->base, MPPHY_PXSRAMCNT(1), 0x00000020, 0x00000020);
				if (data) {
					dev_err(priv->dev,"Timeout waiting for MPPHY_PXSRAMCNT(1) configuration\n");
				}
				data = mp_phy_reg_wait(priv->base, MPPHY_PXRXREQ1(0), 0x00000002, 0x00000000);
				if (data) {
					dev_err(priv->dev,"Timeout waiting for MPPHY_PXRXREQ(0) configuration\n");
				}
				data = mp_phy_reg_wait(priv->base, MPPHY_PXRXREQ1(1), 0x00000002, 0x00000000);
				if (data) {
					dev_err(priv->dev,"Timeout waiting for MPPHY_PXRXREQ(1) configuration\n");
				}
			}
			if(channel_id == 1) {
				data = mp_phy_reg_wait(priv->base, MPPHY_PXSRAMCNT(2), 0x00000020, 0x00000020);
				if (data) {
					dev_err(priv->dev,"Timeout waiting for MPPHY_PXSRAMCNT(2) configuration\n");
				}
				data = mp_phy_reg_wait(priv->base, MPPHY_PXSRAMCNT(3), 0x00000020, 0x00000020);
				if (data) {
					dev_err(priv->dev,"Timeout waiting for MPPHY_PXSRAMCNT(3) configuration\n");
				}
				data = mp_phy_reg_wait(priv->base, MPPHY_PXRXREQ1(2), 0x00000002, 0x00000000);
				if (data) {
					dev_err(priv->dev,"Timeout waiting for MPPHY_PXRXREQ(2) configuration\n");
				}
				data = mp_phy_reg_wait(priv->base, MPPHY_PXRXREQ1(3), 0x00000002, 0x00000000);
				if (data) {
					dev_err(priv->dev,"Timeout waiting for MPPHY_PXRXREQ(3) configuration\n");
				}
			}
			break;
		case 8:
...
static int mp_phy_config_usb(struct phy *phy, int speed)
{
	...
	case SUPER_SPEED_PLUS:
		...
		if (data) {
			dev_err(priv->dev,"Timeout waiting for PSTATE_1_OFFSET(%d) configuration\n",chan->lane_id);
			return data;
		}
		...
		if (data) {
			dev_err(priv->dev,"Timeout waiting for TCA_INTR_STS_OFFSET(%d) configuration\n", chan->lane_id);
			return data;
		}
