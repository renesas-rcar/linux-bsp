...
#define CPG_ZG_PARENT_PLL0	BIT(4)	/* Use PLL0 as ZG clock parent */
...
static unsigned long cpg_zg_parent_pll0_clk_recalc_rate(struct clk_hw *hw,
					   unsigned long parent_rate)
{
	struct cpg_z_clk *zclk = to_z_clk(hw);
	unsigned long prate = parent_rate / zclk->fixed_div;
...
static long cpg_zg_parent_pll0_clk_round_rate(struct clk_hw *hw, unsigned long rate,
				 unsigned long *parent_rate)
{
	struct cpg_z_clk *zclk = to_z_clk(hw);
	unsigned long prate = *parent_rate / zclk->fixed_div;
...
static int cpg_zg_parent_pll0_clk_set_rate(struct clk_hw *hw, unsigned long rate,
			      unsigned long parent_rate)
{
	struct cpg_z_clk *zclk = to_z_clk(hw);
	unsigned long prate = parent_rate / zclk->fixed_div;
...
static const struct clk_ops cpg_zg_parent_pll0_clk_ops = {
	.recalc_rate = cpg_zg_parent_pll0_clk_recalc_rate,
	.round_rate = cpg_zg_parent_pll0_clk_round_rate,
	.set_rate = cpg_zg_parent_pll0_clk_set_rate,
};

static struct clk * __init cpg_zg_parent_pll0_clk_register(const char *name,
						    const char *parent_name,
						    void __iomem *reg,
						    unsigned int div)
{
	...
	init.name = name;
	init.ops = &cpg_zg_parent_pll0_clk_ops;
	init.flags = 0;
	init.parent_names = &parent_name;
	init.num_parents = 1;
	...
	struct raw_notifier_head *notifiers)
{
	...
	case CLK_TYPE_GEN3_ZG:
		if (cpg_quirks & CPG_ZG_PARENT_PLL0)
			return cpg_zg_parent_pll0_clk_register(core->name,
