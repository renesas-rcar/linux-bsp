			      ...
			      struct cc_hw_desc desc[], unsigned int *seq_size)
{
	...
	if (is_aes_ctr_prot) {
		...
		hw_desc_init(&desc[*seq_size]);
		set_din_type(&desc[*seq_size], DMA_DLLI,
			     ctx_p->drvdata->dropped_buffer_dma_addr,
			     sizeof(ctx_p->drvdata->dropped_buffer), NS_BIT);
		set_dout_dlli(&desc[*seq_size],
			      ctx_p->drvdata->dropped_buffer_dma_addr,
			      sizeof(ctx_p->drvdata->dropped_buffer), NS_BIT, 0);
		set_flow_mode(&desc[*seq_size], flow_mode);
		(*seq_size)++;
	}
}
				 ...
				 struct cc_hw_desc desc[], unsigned int *seq_size)
{
	...
	set_dout_dlli(&desc[*seq_size],
		      ctx_p->drvdata->dropped_buffer_dma_addr,
		      sizeof(ctx_p->drvdata->dropped_buffer),
		      NS_BIT, 1);
	set_flow_mode(&desc[*seq_size], flow_mode);
	(*seq_size)++;
}
...
int cc_cipher_free(struct cc_drvdata *drvdata)
{
	...
	list_for_each_entry_safe(t_alg, n, &drvdata->alg_list, entry) {
		...
		if (drvdata->dropped_buffer_dma_addr != 0) {
			dma_unmap_single(dev,
					 drvdata->dropped_buffer_dma_addr,
					 sizeof(drvdata->dropped_buffer),
					 DMA_TO_DEVICE);
			drvdata->dropped_buffer_dma_addr = 0;
		}
	}
	return 0;
}
...
int cc_cipher_alloc(struct cc_drvdata *drvdata)
{
	...
	int rc = -ENOMEM;
	int alg;
	drvdata->dropped_buffer_dma_addr = dma_map_single(dev,
							  &drvdata->dropped_buffer,
							  sizeof(drvdata->dropped_buffer),
							  DMA_TO_DEVICE);
	if (drvdata->dropped_buffer_dma_addr == 0) {
	...
	cc_iowrite(drvdata, CC_REG(DSCRPTR_FILTER_DROPPED_ADDRESS),
		   (drvdata->dropped_buffer_dma_addr & UINT_MAX));
	/*set 32 high bits*/
	cc_iowrite(drvdata, CC_REG(DSCRPTR_FILTER_DROPPED_ADDRESS_HIGH),
		   (drvdata->dropped_buffer_dma_addr >> 32));
