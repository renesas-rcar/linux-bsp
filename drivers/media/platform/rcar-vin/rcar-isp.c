...
{
	int i;
	u32 dt = 0, dt_code = 0x24;	/* DT code = RGB888 (0x24) */
	...
	dt = (dt_code << 24) | (dt_code << 16) | (dt_code << 8) | dt_code;
	isp_write(isp, DT_CODE03_ALL_EN | dt, ISPCS_DT_CODE03_CH(ch));
...
int rcar_isp_init(struct rcar_isp_device *isp)
{
	int ch, ch_s, ch_e, vc;
	u32 dt_num = 36; /* RGB888 */

	ch_s = isp->info->ch_start;
	ch_e = isp->info->ch_end;

	for (ch = ch_s, vc = 0; ch < ch_e && vc < 4; ch++, vc++)
		rcar_isp_pre_init(isp, ch, vc);

	isp_write(isp, 0xc97efb69, ISPWP_CTRL);
	isp_write(isp, isp_read(isp, ISPINPUTSEL0) | (0x04 << 8),
		  ISPINPUTSEL0);
	isp_write(isp, isp_read(isp, ISP_PADDING_CTRL) | 0x20,
		  ISP_PADDING_CTRL);
	isp_write(isp, 0x15151515, ISPPROCMODE_DT(dt_num));
	isp_write(isp, 0xc97efb69, ISPWP_CTRL);
