...
int rvin_v4l2_probe(struct rvin_dev *vin)
{
	...
	if (vin->info->use_mc) {
		...
		if (vin->vdev.tvnorms == 0) {
			/* Disable the STD API if no tvnorms defined */
		...
		/* Add the controls
		 *
		 * Currently the subdev with the largest number of controls
		 * (13) is ov6550. So let's pick 16 as a hint for the control
		 * handler. Note that this is a hint only: too large and you
		 * waste some memory, too small and there is a (very) small
		 * performance hit when looking up controls in the internal
		 * hash.
		 */
