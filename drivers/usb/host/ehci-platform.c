...
static void quirk_poll_timer(unsigned long data)
{
	struct ehci_platform_priv *priv = (struct ehci_platform_priv *)data;
	struct ehci_hcd *ehci = container_of((void *)priv, struct ehci_hcd,
					     priv);
...
static void quirk_poll_init(struct ehci_platform_priv *priv)
{
	...
	setup_timer(&priv->poll_timer, quirk_poll_timer, (unsigned long)priv);
	mod_timer(&priv->poll_timer, jiffies + HZ);
}
