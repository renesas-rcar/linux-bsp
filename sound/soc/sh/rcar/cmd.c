			 ...
			 struct rsnd_priv *priv)
{
	...
	if (rsnd_is_gen5(priv)) {
		if (!mix) {
			/* Now, there is no limitation of what cmd input will be used
			 * for each SRC
			 * e.g. On gen3:
			 *	cmdin_ctu0 for SRC3/6
			 *	cmdin_ctu1 for SRC4/9
			 *	...
			 * On gen5, each cmdin_ctux can access to SRC0-9 and CMD from the other audio subsystem.
			 * Therefore, by default the cmdin_ctu0 will be used if not mix case.
			 */
